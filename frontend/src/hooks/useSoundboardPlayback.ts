...
export function useSoundboardPlayback(opts: UseSoundboardPlaybackOpts) {
  ...
  const playSlot = useCallback(
    async (slot: Slot, options: PlaybackOptions = {}) => {
      ...
      // async caller (the audit runner) captures this closure before the socket
      // opens, so the state values would be stale-false even though PP is ready.
      if (!ws.isReadyForAudio()) {
        setError(ws.connected
          ? "PersonaPlex is still warming up — wait for it to be ready "
